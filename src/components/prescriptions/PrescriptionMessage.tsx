...
export default function PrescriptionMessage({
  ...
  return (
    <div
      className={`inline-flex flex-col rounded-3xl px-5 py-3 text-sm leading-6 shadow-theme-sm ${
        isOwn
    ...
    >
      ...
      {medicine_details && (
        <div className="mb-3 whitespace-pre-wrap text-sm leading-relaxed opacity-90">
          {medicinePreview}
        </div>
      )}
      ...
      {text && text !== 'Prescription' && (
        <div className="mb-3 border-t border-white/20 pt-3 text-sm opacity-90">
          {text}
        </div>
      )}
